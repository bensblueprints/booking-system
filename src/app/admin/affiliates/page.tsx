    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white border border-slate-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <div>
            <div className="text-xs text-slate-700">Active Affiliates</div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <div>
            <div className="text-xs text-slate-700">Commission Owed</div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <div>
            <div className="text-xs text-slate-700">Total Affiliates</div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={9} className="px-4 py-8 text-center text-slate-700">Loading...</td></tr>
              ) : affiliates.length === 0 ? (
                <tr><td colSpan={9} className="px-4 py-8 text-center text-slate-700">No affiliates yet</td></tr>
              ) : (
                affiliates.map((a) => (
                  <tr key={a.id} className="border-b border-slate-100 hover:bg-slate-100">
                    ...
                    <td className="px-4 py-3 text-slate-700">{a.bookings_count || 0}</td>
                    <td className="px-4 py-3 text-slate-700">{formatCurrency(a.revenue)}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <button onClick={() => viewBookings(a)} className="p-1.5 hover:bg-slate-100 rounded-lg transition-colors" title="View Bookings">
                          <Eye className="w-4 h-4 text-slate-700" />
                        </button>
                        <button onClick={() => openEdit(a)} className="p-1.5 hover:bg-slate-100 rounded-lg transition-colors" title="Edit">
                          <Pencil className="w-4 h-4 text-slate-700" />
                        </button>
                        <button onClick={() => handleDelete(a.id)} className="p-1.5 hover:bg-slate-100 rounded-lg transition-colors" title="Delete">
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editingId ? "Edit Affiliate" : "Add Affiliate"}>
        <div className="space-y-4">
          <div>
            ...
            <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm text-slate-700 mb-1">Email *</label>
            <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-sm text-slate-700 mb-1">Affiliate Code</label>
            <div className="flex gap-2">
              <input type="text" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} className="flex-1 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm font-mono" />
              <button type="button" onClick={() => setForm({ ...form, code: generateCode() })} className="px-3 py-2 bg-slate-200 hover:bg-white/20 rounded-lg text-sm transition-colors">Generate</button>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-slate-700 mb-1">Commission Type</label>
              <select value={form.commission_type} onChange={(e) => setForm({ ...form, commission_type: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm">
                <option value="percent">Percentage</option>
                <option value="fixed">Fixed Amount</option>
              </select>
            </div>
            <div>
              <label className="block text-sm text-slate-700 mb-1">Commission Value *</label>
              <input type="number" min="0" step="0.01" value={form.commission_value} onChange={(e) => setForm({ ...form, commission_value: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm" placeholder={form.commission_type === "percent" ? "10" : "5.00"} />
            </div>
          </div>
      ...
      <Modal open={showBookings} onClose={() => setShowBookings(false)} title={`Bookings — ${selectedAffiliate?.name || ""}`} wide>
        {bookings.length === 0 ? (
          <div className="py-8 text-center text-slate-700">No bookings found</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
              ...
              <tbody>
                {bookings.map((b) => (
                  <tr key={b.id} className="border-b border-slate-100">
                    <td className="py-2 font-mono text-slate-900">{b.booking_ref}</td>
                    <td className="py-2 text-slate-700">{b.customer_name}</td>
                    <td className="py-2 text-slate-700">{b.product_name}</td>
                    ...
                    <td className="py-2">
                      {b.paid ? (
                        ...
                        <span className="text-slate-700 text-xs">Unpaid</span>
                      )}
                    </td>
                    <td className="py-2 text-slate-700">{new Date(b.created_at).toLocaleDateString()}</td>
                    <td className="py-2">
                      {!b.paid && (
